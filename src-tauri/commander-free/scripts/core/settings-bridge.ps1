# settings-bridge.ps1
# ─────────────────────────────────────────────────────────────────────
# PowerShell bridge for reading/writing the unified settings.json.
# Loaded by modules that need to check desired state or sync back
# applied state after OS-level changes.
# ─────────────────────────────────────────────────────────────────────

function Get-WCSettingsPath {
    $appData = [Environment]::GetFolderPath('ApplicationData')
    return Join-Path $appData "WinCommander\settings.json"
}

function Get-WCSettings {
    <#
    .SYNOPSIS
        Reads the unified settings.json and returns as a PSObject.
    .DESCRIPTION
        Returns $null if file doesn't exist (pre-migration state).
    #>
    $path = Get-WCSettingsPath
    ...
    try {
        ...
        Write-Warning "Failed to read settings.json: $_"
        return $null
    }
}

function Set-WCSetting {
    <#
    .SYNOPSIS
        Updates a specific setting by dot-path in settings.json.
    .PARAMETER Path
        Dot-separated path, e.g., "privacy.telemetry.windowsDisabled"
    .PARAMETER Value
        The value to set (bool, string, number, array).
    #>
    param(
        [Parameter(Mandatory)]
        [string]$Path,
    ...
    $settingsPath = Get-WCSettingsPath
    if (-not (Test-Path $settingsPath)) {
        Write-Warning "settings.json does not exist yet"
        return
    }
    ...
    for ($i = 0; $i -lt $segments.Count - 1; $i++) {
        $seg = $segments[$i]
        if ($null -eq $current.$seg) {
            # Create intermediate object
            $current | Add-Member -NotePropertyName $seg -NotePropertyValue ([PSCustomObject]@{}) -Force
        }
        $current = $current.$seg
    }
    $lastSeg = $segments[-1]

    if ($current.PSObject.Properties.Name -contains $lastSeg) {
        $current.$lastSeg = $Value
    }
    else {
        $current | Add-Member -NotePropertyName $lastSeg -NotePropertyValue $Value -Force
    }

    # Update lastSeenAt. MUST be an ISO-8601 UTC string to match the Rust
    # Settings struct (last_seen_at: String, written via now_iso8601()).
    # Writing an int64 epoch here made read_settings fail to deserialize
    # ("invalid type: integer, expected a string"), which broke the whole app.
    $settings.lastSeenAt = (Get-Date).ToUniversalTime().ToString("yyyy-MM-dd'T'HH:mm:ss'Z'")

    $settings | ConvertTo-Json -Depth 20 | Set-Content -Path $settingsPath -Force
}

function Get-WCSetting {
    <#
    .SYNOPSIS
        Gets a specific setting by dot-path.
    .PARAMETER Path
        Dot-separated path, e.g., "tweaks.security.defenderDisabled"
    #>
    param(
        [Parameter(Mandatory)]
        [string]$Path
    ...
    $settings = Get-WCSettings
    if ($null -eq $settings) { return $null }

    $current = $settings
    foreach ($seg in ($Path -split '\.')) {
        if ($null -eq $current.$seg) { return $null }
        $current = $current.$seg
    }
    return $current
}

function Test-WCSettingLocked {
    <#
    .SYNOPSIS
        Checks if a setting path is locked by admin policy.
    .PARAMETER Path
        The dot-path to check, e.g., "privacy.telemetry"
    .OUTPUTS
    ...
    if ($settings.policy.syncMode -ne 'managed') { return $false }

    foreach ($locked in $settings.policy.lockedPaths) {
        if ($Path.StartsWith($locked)) { return $true }
    }
    return $false
}

function Sync-WCSettingFromRegistry {
    <#
    .SYNOPSIS
        Reads the actual state from registry and updates settings.json
        to reflect reality. Used during migration and drift detection.
    .PARAMETER SettingPath
        The settings.json dot-path to update.
    .PARAMETER RegistryPath
        The full registry path to read from.
    .PARAMETER PropertyName
        The registry property name.
    .PARAMETER InvertLogic
        If true, a registry value of 1 means disabled (inverted).
    #>
    param(
        [string]$SettingPath,
        [string]$RegistryPath,
    ...
    try {
        $val = Get-ItemProperty -Path $RegistryPath -Name $PropertyName -ErrorAction Stop
        $regVal = $val.$PropertyName
        ...
        Set-WCSetting -Path $SettingPath -Value $boolVal
    }
    catch {
        # Registry key doesn't exist — leave setting as default
    }
}
