...
pub async fn vault_call(
    ...
    let result = async {
        timeout(BROKER_TIMEOUT, pipe.connect())
            .await
        ...
        timeout(BROKER_TIMEOUT, write_envelope(&mut pipe, &hello))
            .await
            .map_err(|_| "broker_timeout")?
            .map_err(|_| "broker_io")?;
        let ack = timeout(BROKER_TIMEOUT, read_envelope(&mut pipe))
            .await
            .map_err(|_| "broker_timeout")?
            .map_err(|_| "broker_io")?;
        ...
        .sign(&session_token);
        timeout(BROKER_TIMEOUT, write_envelope(&mut pipe, &request))
            .await
            .map_err(|_| "broker_timeout")?
            .map_err(|_| "broker_io")?;
        let reply = timeout(BROKER_TIMEOUT, read_envelope(&mut pipe))
            .await
            .map_err(|_| "broker_timeout")?
            .map_err(|_| "broker_io")?;
            ...
            .map_err(|_| "broker_hmac")?
        {
            ...
            Envelope::Error(error) if error.kind == "vault_acl_readback_failed" => {
                Err("vault_acl_readback_failed")
            }
            Envelope::Error(error) if error.kind == "vault_acl_apply_failed" => {
                Err("vault_acl_apply_failed")
            }
...
#[cfg(windows)]
fn spawn_pro_in_target_session(
    session_id: u32,
    pipe: &str,
    token: &str,
