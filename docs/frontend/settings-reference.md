...
The detailed unknown-keyboard approval procedure is private Pro operator
documentation; this public reference records only the settings contract.
...
- **Concealment:** `hideNotificationBell`, `hiddenSidebarActions`, `permanentlyHiddenPanels`, `borrowedHidden`, `hideLicensePanel`, `lockedPanelIds`, `unlockKeyword`/`lockKeyword`, `lockPanelOnClose`.
  - On a new install, Borrowed Mode hides Dashboard Risk Matrix, Dashboard More Products, Alerts and Processes, pop-up alerts, desktop alerts, and the sidebar persona/interface controls. AI Advisor starts hidden everywhere; Secret Settings can reveal any of these choices.
