...
The generated catalog contains 1,246 entries: 803 backend-script commands and 443 Tauri handlers. Four Tauri handlers are debug-only, so the shipped release binary executes 1,242 commands; it retains the four debug-only entries for catalog-drift auditing and refuses them at runtime.
