...
| Unknown-keyboard approval | Reactively disables every unambiguous candidate HID function, verifies Windows reports containment, then asks the operator to complete a backend-bound randomized click challenge before Allow once. Always trust requires a stable serial identity shared by the contained group, two independent challenges, and dedicated challenge provenance; old/manual allow-list entries do not inherit that trust. It is human-presence friction, not proof that a separate trusted mouse clicked. | Gate on/off; approval expiry; Allow once; Always trust when eligible; Keep blocked. See [operator guide](usb-keyboard-approval.md). |
