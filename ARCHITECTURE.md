...
- **`wci-verify.exe`** — an independently distributable CLI verifier for case bundles, delivery receipts, and detached report-integrity signatures.
  ...
  | `stego_backup.rs` | Existing VeraCrypt `.hc`/`.tc` snapshots can be attached to a carrier MP4, restored under their embedded original filename, or safely refreshed; version-2 metadata carries bounds and a checksum, while the legacy empty-container create/extract commands remain supported |
