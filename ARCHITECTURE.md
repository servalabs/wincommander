...
| ArgusSignal fleet channel | Unified wire struct `ArgusSignal` (`fleet-proto/src/lib.rs:837`, re-exported as `wincmd_shared::fleet::ArgusSignal` — `wincmd-shared/src/lib.rs:623` does `pub use fleet_proto as fleet;`, not a separate `fleet.rs` file): `window_start/end`, `kind` ∈ {`dlp_exfil`,`tamper`,`print`,`removable_media`}, `class`, `magnitude`, `severity`, `consent_version`, `disclosure_version` — NO names/paths. Carried in the `POST /v1/agents/checkin` body (HMAC checkin_secret auth; 403 if `consent_version ≠ disclosure_version`). Admin `GET /orgs/{org}/argus`. `ArgusStore` trait + memory + Postgres impls (`fleet-server/src/store/`). `argus_signals.rs` (Pro): bounded enqueue/drain queue drained by `fleet_push` into the check-in body. Migration `0006_argus_signals.sql`. | `src-tauri/fleet-proto/src/lib.rs`, `commander-pro/fleet-server/src/routes/duress.rs`, `commander-pro/fleet-server/migrations/0006_argus_signals.sql`, `commander-pro/commander-pro/src/argus_signals.rs` |
...
- **Argus privacy invariant (design constraint):** all four Argus collectors enforce a single invariant: window titles, exe paths, URLs, filenames, printer names, document names, and usernames are used locally (to compute aggregates) but NEVER serialised onto the `ArgusSignal` wire struct and NEVER sent to the fleet server. The `ArgusSignal` struct in `fleet-proto/src/lib.rs` (re-exported as `wincmd_shared::fleet::ArgusSignal`) carries only scalar `magnitude`, `kind`, `class`, and `severity` — no string fields that could hold identifying content. The fleet server's `POST /v1/agents/checkin` handler additionally gate-closes the argus portion of the body on a `consent_version ≠ disclosure_version` mismatch (403), so a stale or mismatched consent record blocks upload at the server boundary as well.
...
**Agent check-in auth (2026-07-09 unified transport)** — enroll (`POST /v1/agents/enroll`) issues a per-device HMAC `checkin_secret`, returned once; the server stores it KEK-encrypted (AES-256-GCM, `FLEET_CHECKIN_KEK`/`CheckinKek`) rather than a hash. Every subsequent call is `POST /v1/agents/checkin`, authenticated by HMAC-SHA256 over `"{device_id}:{ts}:{nonce}"` (ts-window + nonce-replay defence) — no bearer token, no device keypair. The agent (`commander-pro/fleet_push.rs`) holds the `checkin_secret` in its `AgentSession` and signs each check-in; the response returns `{commands, all_clear, config_epoch}` in one round-trip.
...
- **Bundling:** Tauri NSIS/MSI, `installMode: perMachine`, `createUpdaterArtifacts: true`. Resources bundle the `.enc` modules + helper EXEs (`tauri.conf.json`). Updater endpoint `winupdates.servalabs.com/free/latest.json`, minisign pubkey embedded.
