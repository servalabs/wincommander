...
| Startup and live UI state | Settings hydrate from the local cache before background system probes; `startupDataState` distinguishes cached, refreshing, ready, and stale data. App-wide settings/navigation live in `AppContext`; the dashboard-only two-second CPU/RAM/disk samples and motion resolution use narrow providers so gauge updates do not redraw unrelated panels. | `src/context/AppContext.tsx`, `src/context/LiveMetricsContext.tsx`, `src/hooks/useMotionPreference.ts` |
...
| Fleet Vault service | Service-owned authorization, policy, and mount lifecycle. The renderer receives only an authorized entry projection and bounded mount outcomes; it never receives raw container paths, SIDs, or ACLs. | `src-tauri/commander-svc/`, `src-tauri/commander-free/src/vault_access.rs`, `src/hooks/useVaultAccess.ts` |
