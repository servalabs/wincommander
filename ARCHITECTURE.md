...
| Startup and live UI state | Settings hydrate from the local cache before background system probes; normal cached launches can paint without waiting for the decorative splash. One app-wide startup coordinator shares duplicate work and allows only one expensive launch probe/scan at a time. Panel and Disk Cleanup warming is intent/idle-led. Frequent live metrics share a bounded five-second native disk snapshot while CPU/RAM remain live; identity, licence, portable-state, settings, and mutation inputs are not cached there. A bounded native trace uses one monotonic clock for allowlisted native phases and frontend job milestones; it stores no paths, settings values, command arguments, licence material, or error text. | `src/context/AppContext.tsx`, `src/services/startupCoordinator.ts`, `src/events/startup.ts`, `src-tauri/commander-free/src/system_metrics.rs`, `src-tauri/commander-free/src/startup_trace.rs` |
