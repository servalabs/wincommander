...
flowchart LR
  ...
  subgraph binaries["Per-machine installation"]
    direction TB
    FREE["wincommander-free.exe<br/>(commander-free, AGPL)<br/>UI · free tier · licence · broker<br/>highestAvailable: caller token when not elevated"]
    PRO["wincommander-pro.exe<br/>(commander-pro, paid)<br/>headless, paid-tier handlers"]
    SVC["wincommander-svc.exe<br/>LocalSystem WinCommanderSvc<br/>Fleet Vault policy + mount lifecycle"]
  end
...
| Startup and live UI state | Coordinates cached settings hydration, launch probes, panel warming, shared metric snapshots, and a privacy-bounded native trace. See [PERFORMANCE.md](PERFORMANCE.md) for responsiveness behavior and measurement status. | `src/context/AppContext.tsx`, `src/services/startupCoordinator.ts`, `src/events/startup.ts`, `src-tauri/commander-free/src/system_metrics.rs`, `src-tauri/commander-free/src/startup_trace.rs` |
...
| Endpoint control plane (in progress) | `commander-svc` declares three background loops — `fleet_conn_loop`, `reconciler_loop`, `command_worker_loop` — that are still `sleep(30)` stubs, alongside a durable signed-policy store (independent signature verification, monotonic version guard, atomic swap with last-valid retention) and a Pro broker that today serves only the vault verbs. The intended destination is that the service owns the fleet connection, desired state, observed state, and the difference between them, with the sidecar reduced to a headless executor it parents. Until those loops are implemented, the fleet agent runs inside the Pro sidecar that `commander-free` spawns, so fleet check-ins and policy reapplication stop when the desktop app is not running, and the state Fleet reports comes from a local OS probe rather than from state WinCommander confirms it applied — see [SECURITY.md](SECURITY.md) for the operational limits that follow. Detailed sequencing is Pro-internal. | `src-tauri/commander-svc/src/main.rs`, `policy_store.rs`, `pro_broker.rs`, `src-tauri/wincmd-shared/src/svc.rs` |
| Fleet Vault service | Service-owned authorization, policy, and mount lifecycle. Apply/mount/unmount share one policy generation; durable recovery failures are scoped per entry; boot cleanup dismounts by internal slot in SYSTEM context; and a shared live mount requires its original SID/session to unmount. Client mutations use unique request IDs, verb-specific deadlines, and report timeout as outcome-unknown. The renderer receives only an authorized entry projection and bounded mount outcomes; it never receives raw container paths, SIDs, or ACLs. | `src-tauri/commander-svc/`, `src-tauri/commander-free/src/svc_client.rs`, `src-tauri/commander-free/src/vault_access.rs`, `src/hooks/useVaultAccess.ts` |
...
| Fleet alert fan-in | One signed SystemState leaf (`security.requireAllDeviceAlertsInFleet`) is the master policy. Device producers resolve `master OR local preference`, normalize to a closed PII-free alert schema, queue through Pro, and batch into the authenticated check-in. Clipboard Guard uses stable event IDs so Free→Pro retries are idempotent. | `commander-free/src/{settings,net_traffic_alert,fleet_agent,paste_monitor}.rs`; `commander-pro/src/{argus_signals,clipboard_guard,handlers}.rs` |
...
| Basic USB timeline (Free) | Neutral WMI attach/detach history, optional local notification, and read-only mounted-volume discovery. It performs no attack classification, timing analysis, trust scoring, transfer intelligence, or device enforcement. | `src-tauri/commander-free/src/usb_guard.rs` |
| USB Guard bridge (Free UI + paid IPC) | Typed `require_paid` wrappers and UI for Pro-owned transfer metering, low-confidence HID timing alerts, trust/policy, reactive isolation, and unknown-keyboard approval. Positive approval is sent one click at a time; the WebView never receives a raw PnP InstanceId for the decision. Expiry-safe stop calls remain available after entitlement loss. | `src-tauri/commander-free/src/usb_guard.rs`; `src/hooks/usbHidApprovalIpc.ts`; `src/context/UsbHidApprovalContext.tsx`; `src/components/shared/UsbHidApprovalDialog.tsx` |
| Monitor Operations Center | One bounded, content-free device snapshot combines local monitor status with the authenticated Pro monitor inventory. It reports running/alert/degraded/stale/unavailable/locked state, recent and active counts, cadence, and generic error codes; it never forwards raw event payloads, paths, usernames, processes, or peers. A local status probe has a five-second deadline and the Pro inventory has a ten-second deadline; timed-out checks become unavailable rather than delaying the whole view. Stateful Pro lifecycle/history/config calls require the durable Fleet-agent session so status and collector state remain co-located; an unavailable agent is reported explicitly, never replaced with a non-authoritative pool worker. | `src-tauri/commander-free/src/monitoring*.rs`, `src-tauri/commander-free/src/sidecar.rs`, `src/hooks/useMonitoringOverview.ts`, `src/panels/privacy/MonitoringOperationsSection.tsx`, `../wincommander-pro/commander-pro/src/monitoring*.rs` |
...
- **`wci-verify.exe`** — an independently distributable CLI verifier for case bundles, delivery receipts, and detached report-integrity signatures.
  ...
  | `usb_guard.rs` | Machine-scoped USB identity/timeline intelligence, transfer metering, low-confidence timing anomalies, reactive post-attach isolation, and backend-owned unknown-keyboard approval. It contains and reads back the whole verified HID group before offering a decision; permanent trust is separate, challenge-provenanced state. |
...
Contributor setup, development, and verification commands are maintained in
[AGENTS.md](AGENTS.md). Public release-facing history is maintained in
[CHANGELOG.md](CHANGELOG.md); private release operations and acceptance evidence
remain in `wincommander-pro`.
