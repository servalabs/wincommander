...
- **Bundling:** Tauri NSIS/MSI, `installMode: perMachine`, `createUpdaterArtifacts: true`. Resources bundle the `.enc` modules + helper EXEs (`tauri.conf.json`). Free updater endpoint is the public GitHub Release `latest.json`; minisign pubkey is embedded.
