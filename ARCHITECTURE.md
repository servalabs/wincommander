...
flowchart LR
  ...
  subgraph binaries["Per-user desktop installation"]
    direction TB
...
| Content-search engine (`wincmd-search`) | Schema v7 Tantivy index. Ordinary folders use a resident crawler/watcher in the per-user host index; private folders use incremental reconciliation and short-lived readers/writers inside each mounted volume. Restricted APIs intersect query scope with current configured roots and apply a native path policy; an empty allowed-root set denies all results and previews. Read-only private opens neither create files nor migrate schema. Ordinary shared readers retain metadata locking. | `src-tauri/wincmd-search/` |
| File-search IPC (Free) | `file_search` routes ordinary/private shards and publishes an ordinary-only scope manifest. `content_privacy_status` exposes a mount generation for frontend response validation. Settings retain native stable-volume bindings; private job/index handles are short-lived. Everything rows require a currently verified ordinary path; unverified aggregate totals are unavailable. Native volume inspection and the dismount operation gate live in `wincmd-volume`. Search handoff is memory-only; private paths are excluded from persistent frecency. | `src-tauri/commander-free/src/file_search/`, `src-tauri/wincmd-volume/`, `src-tauri/commander-free/src/backend.rs` |
...
- **`wci-verify.exe`** — an independently distributable CLI verifier for case bundles, delivery receipts, and detached report-integrity signatures.
  ...
  | `stego_backup.rs` | Existing VeraCrypt containers, including extensionless NTFS containers, can be attached to a carrier MP4 or update a selected backup video in place after explicit confirmation; restores retain the embedded original filename and version-2 metadata carries bounds and a checksum |
