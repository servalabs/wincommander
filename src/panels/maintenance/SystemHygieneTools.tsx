  ...
  return <div className="flex flex-col gap-4">
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">{tools.busy && tools.tool !== "environment" && <Button size="icon" variant="outline" onClick={() => void tools.cancel()} title={`Cancel ${tools.tool} scan`} aria-label={`Cancel ${tools.tool} scan`}><Icon icon="stop" /></Button>}{items && <Badge tone="accent">{items.length} candidate{items.length === 1 ? "" : "s"}</Badge>}<Button size="sm" variant="primary" className="ml-auto" disabled={tools.busy} onClick={() => void tools.scan()} title={scanLabel} aria-label={scanLabel}><Icon icon={tools.busy || items ? "refresh" : "search"} className={tools.busy ? "animate-spin" : undefined} />{scanLabel}</Button></div>
      </CardContent>
    </Card>
