  ...
  return (
    <div className="fleet-table-wrap">
      <table className="fleet-policy-table">
        <thead><tr>
          <th>Windows user</th><th>Groups</th><th>Volume</th><th>Effective</th><th>See backing</th><th>Mount</th>
          <th>Decrypt</th><th>Content</th><th>Other session</th>
        </tr></thead>
        <tbody>{rows.map(row => (
          <tr key={`${row.userClass}-${row.volume}`}>
            <td>{row.userClass}</td><td>{row.groups}</td><td>{row.volume}</td><td>{row.effectiveAccess}</td><td>{mark(row.canSeeBacking)}</td>
