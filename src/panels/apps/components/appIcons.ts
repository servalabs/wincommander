...
//      manifest (now sourced mainly from assets-shared/softwares/*, exposed via
//      src/assets.ts). If the asset
//      ships, the <img> renders. If not, onError swaps in a Blueprint icon
//      picked by category.
//   3. Drop a new SVG into assets-shared/softwares/<slug>.svg to "light up"
...
export function brandIconCandidates(id: string): string[] {
  ...
  // Run `bun run fetch-icons` to add brand icons into assets-shared/softwares/.
  const out: string[] = [];
