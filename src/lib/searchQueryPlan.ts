...
const TYPE_KINDS = ["documents", "pdf", "excel", "word", "text", "slides", "images", "videos", "audio", "archives", "apps", "code"] as const;
type TypeKind = (typeof TYPE_KINDS)[number];

const TYPE_EXTENSIONS: Record<TypeKind, readonly string[]> = {
  documents: FILE_TYPE_EXTENSIONS.documents,
  pdf: ["pdf"], excel: ["xls", "xlsx", "csv"], word: ["doc", "docx"], text: ["txt", "md", "rtf"], slides: ["ppt", "pptx"],
  images: FILE_TYPE_EXTENSIONS.images, videos: FILE_TYPE_EXTENSIONS.videos, audio: FILE_TYPE_EXTENSIONS.audio,
  archives: FILE_TYPE_EXTENSIONS.archives, apps: FILE_TYPE_EXTENSIONS.apps, code: FILE_TYPE_EXTENSIONS.code,
};
...
const TYPE_NOUNS: Record<TypeKind, string> = {
  documents: "documents",
  pdf: "PDFs", excel: "Excel files", word: "Word files", text: "text files", slides: "slides",
  images: "images",
  videos: "videos",
  audio: "audio files",
  archives: "archives",
...
const TEXT_BEARING: readonly TypeKind[] = ["documents", "pdf", "excel", "word", "text", "slides", "code"];
...
function extsFor(kinds: Set<ChipKind>): string[] {
  ...
  for (const kind of TYPE_KINDS) {
    ...
    for (const ext of TYPE_EXTENSIONS[kind]) out.add(ext);
  }
