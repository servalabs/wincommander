...
const ALL_KINDS: ChipKind[] = [
  "folders", "files", "images", "videos", "audio", "documents", "pdf", "excel", "word", "text", "slides", "code", "archives", "apps",
...
describe("promote / demote round-trip", () => {
  ...
  it("removes a dropdown storage scope without adding its path to the query", () => {
    const state = addChip(q("part"), "in", {
      path: "D:\\Personal\\Downloads",
      pathLabel: "Downloads",
      source: "",
    });
    expect(demoteLastChip(state)).toEqual({ chips: [], text: "part" });
  });
});
