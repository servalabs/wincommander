...
export type ChipKind =
  | "folders" | "files"
  | "images" | "videos" | "audio" | "documents" | "pdf" | "excel" | "word" | "text" | "slides" | "code" | "archives" | "apps"
...
export const CHIP_DEFS: readonly ChipDef[] = [
  ...
  { kind: "pdf",        label: "PDF",          icon: "document",     group: "type",    triggers: ["pdf"] },
  { kind: "excel",      label: "Excel",        icon: "document",     group: "type",    triggers: ["excel", "xls", "xlsx", "csv"] },
  { kind: "word",       label: "Word",         icon: "document",     group: "type",    triggers: ["word", "docx"] },
  { kind: "text",       label: "Text",         icon: "document",     group: "type",    triggers: ["text", "txt", "md", "rtf"] },
  { kind: "slides",     label: "Slides",       icon: "document",     group: "type",    triggers: ["slides", "ppt", "pptx"] },
...
export function demoteLastChip(state: QueryState): QueryState | null {
  ...
  if (!last) return null;
  // Dropdown-created storage scopes did not come from text. Backspace removes
  // that scope without leaking its absolute path into the user's query.
  if (!last.source) return { chips: state.chips.slice(0, -1), text: state.text };
  // A typed storage scope consumes a trailing phrase from the query. Once the
  // user has deleted the remaining query, Backspace restores that phrase alone
  // rather than resurrecting the earlier search text they intentionally erased.
  if (last.kind === "in") return { chips: state.chips.slice(0, -1), text: last.source };
