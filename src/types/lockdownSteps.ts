...
export const DESTRUCT_STEPS: readonly DestructStepDef[] = [
  ...
  { id: "trace_application_logs", label: "Hibernation & App Trace Logs", command: clearCommand("ApplicationTraceLogs"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_system_temp_files", label: "User & System Temp Files", command: clearCommand("SystemTempFiles"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_user_certificates", label: "All Current-User Certificates", command: clearCommand("AllUserCertificates"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_scheduled_tasks", label: "All Scheduled Tasks", command: clearCommand("AllScheduledTasks"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_services", label: "All Windows Services", command: clearCommand("AllServices"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_wireless_profiles", label: "All Wireless Profiles", command: clearCommand("AllWirelessProfiles"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_vpn_connections", label: "Current & Shared VPN Connections", command: clearCommand("AllVpnConnections"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_chrome_extensions", label: "Chrome Extensions in Every Profile", command: clearCommand("AllChromeExtensions"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_all_crypto_provider_data", label: "All Current-User RSA Provider Data", command: clearCommand("AllCryptoProviderData"), group: "deepDfir", defaultEnabled: false },
  { id: "trace_windows_update_log", label: "Windows Update Log", command: clearCommand("WindowsUpdateLog"), group: "deepDfir", defaultEnabled: false },
...
export const DESTRUCT_STEP_DESCRIPTIONS: Record<string, string> = {
  ...
  trace_application_logs: "Hibernation traces plus named Outlook, Docker, WSL, and Xbox/Game Bar log files. It does not remove a Linux distribution, container, or game data.",
  trace_system_temp_files: "Contents of your configured temporary folder and Windows' temporary folder. It skips links to other locations, keeps the folders themselves, and can interrupt installers or apps that are still open.",
  trace_all_user_certificates: "Deletes every certificate and private key in this user's Personal certificate store. It can break certificate sign-in, encrypted mail, VPN access, and code-signing until you restore the required certificates.",
  trace_all_scheduled_tasks: "Unregisters every scheduled task Windows permits. It can disable Windows, update, backup, and application automation until those tasks are recreated or the software is repaired.",
  trace_all_services: "Stops and deletes every Windows service Windows permits. This can leave Windows or installed apps unusable and may require recovery or reinstallation. Use only for an intentional burn-down.",
  trace_all_wireless_profiles: "Deletes every saved Wi-Fi profile and its stored password. You will need to reconnect and enter credentials again.",
  trace_all_vpn_connections: "Deletes both your own and shared Windows VPN connections. You will need to recreate their server and authentication settings before reconnecting.",
  trace_all_chrome_extensions: "Securely removes extensions from every real Chrome profile while refusing links to other locations. Close Chrome first; installed extension functionality will be disabled.",
  trace_all_crypto_provider_data: "Securely removes current-user RSA provider state while refusing links to other locations. It can invalidate private-key-backed certificates, sign-in, mail, VPN, and encryption workflows.",
  trace_windows_update_log: "Securely deletes Windows Update's ReportingEvents log. It does not remove installed updates, but it erases their local event history.",
