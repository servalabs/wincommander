...
import { buildTraceView, formatTraceCell, humanizeTraceKey, paginateTraceRows, traceCellText, traceRowsToTsv, visibleTraceColumns } from "./traceTable";
...
describe("trace table view model", () => {
  ...
  test("expands nested sample arrays while retaining parent forensic context", () => {
    ...
    expect(samples?.columns).toEqual(["Name", "Path", "ID", "Category", "Count"]);
  ...
  test("turns legacy flattened strings into useful fallback columns", () => {
    ...
    expect(view.datasets[0].columns).toEqual(["Entry", "Name", "Destination", "Source", "Value"]);
  ...
  test("never falls back to a one-column forensic dataset", () => {
    ...
    expect(plainText.datasets[0].columns).toEqual(["Entry", "Type"]);
  ...
  test("normalizes technical field labels and display values", () => {
    ...
    expect(formatTraceCell(50, "Size KB")).toBe("0.05 MB");
    expect(formatTraceCell(1_024_000, "Size KB")).toBe("1.00 GB");
    expect(formatTraceCell("2026-08-01 21:14:52", "Modified")).toMatch(/2026/);
  });

  test("hides columns that are empty or zero for every row", () => {
    const columns = visibleTraceColumns(["Name", "Status", "Count"], [
      { Name: "alpha", Status: "—", Count: 0 },
      { Name: "beta", Status: null, Count: "0" },
    ]);
