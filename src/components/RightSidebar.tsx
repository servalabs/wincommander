...
export default function RightSidebar() {
    ...
    const [qmEditing, setQmEditing] = useState<{ idx: number | 'new'; path: string; letter: string } | null>(null);
    const [qmSaving, setQmSaving] = useState(false);
    type QmSlot = { filePath: string; driveLetter: string };
    ...
    const handleQmOpen = useCallback(() => {
        setQmPassword('');
        setQmSelectedIdx(0);
        setQmEditing(quickMountSlots.length === 0 ? { idx: 'new', path: '', letter: nextFreeLetter() } : null);
        setQmOpen(true);
    ...
    const handleQmSaveSlot = useCallback(async () => {
        ...
        if (qmEditing.idx === 'new') next.push({ filePath: path, driveLetter: letter });
        else next[qmEditing.idx as number] = { filePath: path, driveLetter: letter };
    ...
    const handleQmMount = useCallback(async () => {
        ...
        try {
            const r = await mountVolume(slot.filePath, slot.driveLetter, qmPassword);
            if (r?.success) {
    ...
    const handleDismountClick = () => {
        ...
        void handleAction("dismount", async () => {
            if (encryptionAvailable) {
                await dismountAllVolumes();
            }
            if (ramDiskAvailable) {
                const ram = await removeAllRamDisks();
                const ramData = ram?.data as { status?: string; error?: string } | undefined;
                if (!ram?.success) {
                    throw new Error(ram?.error || "Failed to dismount RAM disks.");
                }
                if (ramData?.status === "error" && !String(ramData.error ?? "").toLowerCase().includes("not installed")) {
    ...
    const handleAction = async (action: string, handler: () => Promise<any>) => {
                            ...
                            <>
                                <div className="qm-title-row">
                                    ...
                                    <span className="qm-title">
                                        {qmEditing.idx === 'new' ? 'Add Vault' : 'Edit Vault'}
                                    </span>
                                </div>
                            ...
                            <>
                                <div className="qm-title-row">
                                    ...
                                    <button type="button" className="qm-add-btn"
                                        onClick={() => setQmEditing({ idx: 'new', path: '', letter: nextFreeLetter() })}>
                                            ...
                                            <div className="qm-select-row">
                                                ...
                                                >
                                                    {quickMountSlots.map((slot, idx) => {
                                                        const name = slot.filePath.split(/[\\/]/).filter(Boolean).pop() || slot.filePath;
                                                        return (
                                                ...
                                                <button type="button" className="qm-slot-edit-btn" title="Edit"
                                                    onClick={() => {
                                                        ...
                                                        if (s) setQmEditing({ idx: qmSelectedIdx, path: s.filePath, letter: s.driveLetter });
                                                    }}>
