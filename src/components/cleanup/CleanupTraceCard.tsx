...
interface CleanupTraceCardProps {
  ...
  /** Completed scans render as four compact rows inside one card footprint. */
  compact?: boolean;
}
...
export default function CleanupTraceCard({
  ...
  const showScheduler =
    (!!onSetSchedule || !!onRequestScheduleAccess) && !!category.schedulable;
  ...
  const bandState: BandState = isActionOnly
    ? "action-only"
    : loading
      ? "scanning"
      : error
  ...
  const infoTooltip = (
    <Tooltip>
      ...
      <TooltipContent
        side="bottom"
        align="end"
        className="w-[260px] p-2.5 text-[11px] leading-[1.45]"
      >
        <strong className="mb-1 block text-[var(--color-text-primary)]">
          {category.label}
        </strong>
        {infoText}
      </TooltipContent>
  ...
  const rescanButton =
    !isActionOnly && onLoad && hasScanOutput ? (
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
  ...
  const showFooter =
    isActionOnly || (!loading && hasData && (!!onViewDetails || hasClear));
      ...
      >
        {compact ? (
          <div className="flex h-full items-center gap-1.5 px-2">
            ...
            />
            <span
              className="min-w-0 flex-1 text-left font-bold uppercase leading-tight"
              style={{
                fontSize: 9,
                letterSpacing: "0.4px",
          ...
          <>
            ...
            >
              <Icon
                ...
                style={{
                  color:
                    "color-mix(in srgb, var(--color-success) 60%, var(--color-text-muted))",
                }}
              />
              <div
  ...
  return (
    ...
    >
          ...
          {bandState === "has" ? (
            <span className="flex items-baseline gap-1">
              <span
                ...
                style={{ fontSize: 15, letterSpacing: 0, lineHeight: 1 }}
              >
                {count}
              </span>
      ...
      <div
        ...
        style={{ padding: "9px 12px 6px", gap: 6 }}
      >
        ...
        <div className="flex flex-col gap-0.5 flex-1 overflow-hidden">
          {loading && (
            <div className="flex flex-col gap-1.5 mt-0.5">
              <div
                className="wc-scan-bar w-full h-[3px] rounded-full"
              ...
              <Popover
                ...
                content={
                  <Menu>
                    <MenuItem
                      icon="trash"
                      text={`Clear for ${clearScopeLabel ?? "this user"}`}
                      disabled={clearDisabled}
                      onClick={onClear}
                    />
                    <MenuItem
                      icon="people"
                      text="Clear for all users"
                      intent="danger"
                      disabled={clearDisabled}
