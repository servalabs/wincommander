    ...
    <div className="mt-4 flex flex-col gap-3 border-t border-[var(--border)] pt-4">
      ...
      <div className="grid gap-2 xl:grid-cols-2">
        {ADVANCED_ACTIONS.map((action) => (
          <div key={action.operation} className="flex items-start justify-between gap-3 rounded-[var(--r)] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-3">
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-bold uppercase tracking-[0.3px] text-[var(--text)]">{action.label}</p>
              <p className="mt-1.5 text-[12.5px] leading-relaxed text-[var(--text-dim)]">{action.description}</p>
            </div>
