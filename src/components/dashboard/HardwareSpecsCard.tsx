...
export default function HardwareSpecsCard({ systemInfo, isLoading, expanded = true, onToggle }: HardwareSpecsCardProps) {
    ...
    const { config: alerts, update: updateMetricAlert } = useMetricAlerts();
    ...
    return (
        <div className="hardware-specs-card">
            <div className="card-header hw-card-header">
                ...
                <button
                    type="button"
                    className={`hw-alert-btn ${cpuAlertOn ? "on" : ""}`}
                    onClick={() => {
                        setAlertOpen((o) => !o);
                        if (!cpuAlertOn) {
                            updateMetricAlert("cpu", { enabled: true }).catch((err) => {
                                console.warn("[MetricAlert] enable CPU alert failed", err);
                            });
                        }
                    }}
                    title={cpuAlertOn ? "CPU alert on — configure" : "Configure CPU usage alert"}
                    aria-label="CPU usage alert"
                >
                    {cpuAlertOn ? <Bell size={13} /> : <BellOff size={13} />}
                </button>
            </div>
