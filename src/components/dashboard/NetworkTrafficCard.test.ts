...
describe("NetworkTrafficCard alert configuration", () => {
  test("renders one shared upload/download configuration drawer", async () => {
    ...
    expect((source.match(/metric="upload"/g) ?? []).length).toBe(1);
    ...
    expect(source).toContain('reportToFleetMetrics={["upload", "download"]}');
    expect(source).toContain("showReportToFleet={false}");
  });
});
