...
// Standard volume items (E:) -> totals 24 GB
const standardItems = [
...
//
// These filenames are deliberately the user's OWN confidential work product and
// security credentials. They must NOT depict concealing financial assets, tax
// records, or crypto holdings: the threat this feature is sold against is a
// thief, robber, or extortionist (EULA §18.2), never a lawful demand for access.
// An earlier version listed Bank_Accounts.xlsx, Crypto_Keys.txt, Passport.pdf
// and Tax_Returns_2025.pdf, which read as a how-to for hiding assets and made a
// screenshot of this panel the strongest possible exhibit against us.
const realItems = [
  { name: "Client_Agreements.pdf", size: "2.4 MB", type: "file" },
  { name: "Encryption_Keys.txt", size: "4 KB", type: "file" },
  { name: "ID_Documents.pdf", size: "14 MB", type: "file" },
  { name: "Business_Contracts", size: "156 MB", type: "folder" },
  { name: "Design_Archive.zip", size: "1.8 GB", type: "file" },
  { name: "Customer_Records.db", size: "10.0 GB", type: "file" },
  { name: "Encrypted_Backup", size: "18.0 GB", type: "folder" },
];

type HubState = "locked" | "standard" | "real" | "wrong";

const STATUS_THEMES: Record<HubState, { label: string; icon: typeof Lock }> = {
  locked: { label: "Waiting for credentials", icon: Lock },
  standard: { label: "Standard volume mounted", icon: Folder },
...
export default function TravelHub() {
  ...
  const standardActive = status === "standard";
  const hasResolvedState = standardActive || realActive;
  ...
  return (
    <div className="relative flex h-full min-h-[380px] flex-col rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-4 md:p-5">
      ...
      <div className="mb-4 flex items-center justify-between gap-3 pb-0">
        <div className="min-w-0">
          ...
          <p className="ml-7 mt-0.5 text-[12px] font-bold text-[var(--color-text-secondary)]">
            One physical device · two passwords · two separate volumes
          </p>
        </div>
      ...
      {status !== "locked" && (
        ...
        >
          <div className="flex items-center gap-2">
            <StatusIcon className={`h-4 w-4 ${status === "standard" ? "text-[var(--color-warning)]" : status === "wrong" ? "text-[var(--color-danger)]" : "text-[var(--color-accent)]"}`} />
            <div className="flex items-center gap-2">
              <span className={`inline-block h-2 w-2 rounded-full ${status === "standard" ? "bg-[var(--color-warning)]" : status === "wrong" ? "bg-[var(--color-danger)]" : "bg-[var(--color-accent)]"}`} />
              <span className={`text-[12px] font-black uppercase tracking-[0.18em] ${status === "standard" ? "text-[var(--color-warning)]" : status === "wrong" ? "text-[var(--color-danger)]" : "text-[var(--color-text-primary)]"}`}>
                {theme.label}
              </span>
            </div>
          </div>
      ...
      <div className="flex flex-1 flex-col gap-4">
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          ...
          */}
          {/* The asset key below still reads "decoy": the PNG itself has not been
              renamed in the shared `assets` submodule, which would need a submodule
              bump across this repo and `servalabs.com`. Every visible label already
              says Travel Hub. */}
              ...
              {(typing || hasResolvedState) && (
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase tracking-wider text-[var(--color-text-secondary)]">
                    {realActive ? "Drive X:" : standardActive ? "Drive E:" : "…"}
                  </span>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {realActive ? (
              ...
              >
                ...
                <p className="mt-1 text-[12px] leading-relaxed text-[var(--color-text-muted)] max-w-md mx-auto">
                  Entering standard credentials opens the unencrypted <span className="text-[var(--color-warning)] font-bold">Standard Volume E:</span>.
                ...
                <p className="mt-2 text-[11px] leading-relaxed text-[var(--color-text-muted)] max-w-md mx-auto opacity-75">
                  Built for a stolen or snatched drive &mdash; so a thief or an extortionist sees an ordinary disk.
                  Not for withholding data from a lawful order.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="flex flex-col gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-tertiary)] p-3">
          ...
          <div className="relative flex h-10 w-full overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-primary)]">
            ...
            {/* 3. Space Occupied by Standard Volume Files (Purple) — Completely hidden in real/hidden active state! */}
            {!realActive && (
              <div
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 pt-1.5 border-t border-[var(--color-border)]/50">
            ...
            {!realActive && (
              <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
                ...
                <span>E: Standard Files (24 GB)</span>
              </div>
            )}
            {realActive ? (
