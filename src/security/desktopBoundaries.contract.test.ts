...
describe("desktop trust boundaries", () => {
  ...
  test("RDP configuration uses the OS-resolved PowerShell executable", () => {
    const source = read("src-tauri/commander-free/src/rdp_redirection.rs");
    expect(source).toContain("service_repair_paths::powershell_executable()");
    expect(source).not.toContain('Command::new("powershell.exe")');
  });
