...
2. For desktop operation, use Windows 11 or Windows Server 2019, 2022, or 2025 with Desktop Experience and WebView2 (most features need Administrator). Server Core is not a full GUI target: it has no Explorer/taskbar, and CLI commands work only when WebView2 and that command's prerequisites are installed. See [SECURITY.md](SECURITY.md) for current operational limits.
